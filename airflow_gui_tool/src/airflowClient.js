...
class AirflowClient {
  constructor(airflowConfig = config.airflow) {
    ...
    this.cookieTtlMs = Number(airflowConfig.cookieTtlMs || DEFAULT_COOKIE_TTL_MS);
  ...
  cookieHeader() {
    return [...this.cookieMap.entries()].map(([key, value]) => `${key}=${value}`).join("; ");
  }
  ...
  saveCookiesToStore() {
    if (!this.cookieStorePath || !this.cookieMap.size || !this.cookieExpiresAt) {
      return;
    }
    ...
    fs.writeFileSync(this.cookieStorePath, JSON.stringify(payload, null, 2), "utf8");
  }
  ...
  async login(force = false) {
    ...
    try {
      ...
      if (!postResponse.ok && !isRedirect) {
        ...
        throw new Error(`Airflow 登录失败 (${postResponse.status}): ${body.slice(0, 240)}`);
      }
  ...
  async requestJson(path, options = {}) {
    ...
    if (!response.ok) {
      throw new Error(`${response.status} ${response.statusText}: ${bodyText.slice(0, 500)}`);
    }
    if (!bodyText) {
      return {};
    }
  ...
  async apiGetPaginated(path, params = {}, collectionKey, pageSize = 100) {
    const items = [];
  ...
  async listDags(owner = this.owner) {
    const data = await this.apiGetPaginated("/api/v1/dags", {
      only_active: "true",
    }, "dags");
      ...
      .filter(
        (dag) =>
          ...
          dag.owners.some((item) => {
            ...
            return normalized === ownerNeedle || normalized.includes(ownerNeedle);
          }),
      )
  ...
  async listTasks(dagId) {
    const data = await this.apiGet(`/api/v1/dags/${encodeURIComponent(dagId)}/tasks`);
  ...
  async listDagRuns(dagId, limit = 25) {
    let data;
    try {
      data = await this.apiGet(`/api/v1/dags/${encodeURIComponent(dagId)}/dagRuns`, {
        limit,
        order_by: "-execution_date",
      });
    } catch {
      data = await this.apiGet(`/api/v1/dags/${encodeURIComponent(dagId)}/dagRuns`, { limit });
    }
  ...
  async listTaskTries(dagId, dagRunId, taskId, mapIndex = -1) {
    ...
    const normalizedMapIndex = Number.isFinite(Number(mapIndex)) ? Number(mapIndex) : -1;
    const taskPath =
  ...
  normalizeTaskInstance(task) {
    return {
      ...
      map_index: Number.isFinite(Number(task.map_index)) ? Number(task.map_index) : -1,
  ...
  async setTaskState({ dagId, dagRunId, taskId, mapIndex = -1, newState = "failed", dryRun = false }) {
    const encodedDagId = encodeURIComponent(dagId);
    const encodedDagRunId = encodeURIComponent(dagRunId);
    const encodedTaskId = encodeURIComponent(taskId);
    const normalizedMapIndex = Number.isFinite(Number(mapIndex)) ? Number(mapIndex) : -1;
  ...
  async getTaskLogFromUiEndpoint({ dagId, taskId, executionDate, offset, tryNumber, mapIndex }) {
    ...
    url.searchParams.set("try_number", String(Math.max(1, Number(tryNumber || 1))));
  ...
  async getTaskLogFromApiEndpoint({ dagId, dagRunId, taskId, tryNumber, mapIndex }) {
    const url = new URL(
