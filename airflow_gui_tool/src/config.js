...
function parseEnvUrl(url) {
  ...
  try {
    ...
    console.warn(
      `远程 env 读取失败，已回退本地配置: ${remoteUrl} (${error.message})`
    );
    return {};
  }
}
...
function normalizeBaseUrl(baseUrl) {
  return String(baseUrl || defaultBaseUrl())
    .trim()
    .replace(/\/+$/, "");
}
...
function normalizeAirflowService(rawService = {}, index = 0) {
  ...
  const username = String(
    rawService.username || defaultUsername(baseUrl)
  ).trim();
...
function legacyAirflowService() {
  const baseUrl = normalizeBaseUrl(
    process.env.AIRFLOW_BASE_URL || defaultBaseUrl()
  );
  return normalizeAirflowService({
    id: process.env.AIRFLOW_SERVICE_ID || "main",
    name: process.env.AIRFLOW_SERVICE_NAME || "34.142.225.103",
    label:
      process.env.AIRFLOW_SERVICE_LABEL ||
      `${baseUrl} · ${
        process.env.AIRFLOW_USERNAME || defaultUsername(baseUrl)
      }`,
    baseUrl,
...
module.exports = {
  app: {
    name: "Lovito Task Hub",
    title: "Lovito 任务中心",
  },
  airflow: activeAirflowService,
  airflowServices,
  dagConfFieldMapping,
  loadDagConfFieldMapping: parseDagConfFieldMapping,
